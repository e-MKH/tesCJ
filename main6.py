...
def add_missing_destinations_rows(df: pd.DataFrame, all_destinations: list, depot_coord: dict) -> pd.DataFrame:
    existing_dests = set(df["Destination"])
    missing_dests = set(all_destinations) - existing_dests

    max_route_order = df["Route_Order"].max() if not df.empty else 0
    new_rows = []
    for i, dest in enumerate(sorted(missing_dests)):
        new_rows.append({
            "Vehicle_ID": "Unassigned",
            "Route_Order": max_route_order + i + 1,
            "Destination": dest,
            "Order_Number": "",
            "Box_ID": "",
            "Stacking_Order": "",
...
def main():
    ...
    all_destination_ids = [d["destination_id"] for d in data["destinations"]]
    ...
    df = add_missing_destinations_rows(df, all_destination_ids, location_coords["Depot"])
