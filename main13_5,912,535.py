...
def solve(data, dist_matrix, locations, idx_map):
    ...
    while current_orders:
        ...
        unique_dests = list({o["destination"] for o in current_orders})
        route = get_vrp_route(dist_matrix, idx_map, locations, "Depot", unique_dests)
        if not route:
            break
        dest_rank = {d: i for i, d in enumerate(route)}

        for o in sorted(current_orders, key=lambda x: -dest_rank.get(x["destination"], 999)):
    ...
    print(f"총 비용: {fixed + fuel + total_shuffle_cost:,}원")
