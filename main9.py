...
def solve(data, dist_matrix, locations, idx_map):
    ...
    while current_orders:
        ...
        dest_rank = {d: i+1 for i, d in enumerate(route) if d != "Depot"}
        ...
        # Depot 마지막에 1번으로 추가
        depot_info_last = depot_info.copy()
        depot_info_last["Route_Order"] = max(dest_rank.values()) + 1
        results.append(depot_info_last)
    ...
    shuffle = sum(df[df["Destination"] != "Depot"].groupby("Vehicle_ID").apply(lambda g: g["Z"].rank(method="dense", ascending=False).sum()).astype(int)) * SHUFFLE_COST_PER_BOX

    exec_time = round(time.time() - start_time, 2)
    print(f"처리율: {len(df[df['Destination'] != 'Depot'])} / {len(data['orders'])} = {round(len(df[df['Destination'] != 'Depot'])/len(data['orders'])*100, 2)}%")
